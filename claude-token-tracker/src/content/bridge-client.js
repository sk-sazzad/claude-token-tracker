(() => {
	...
	function injectBridgeOnce() {
		...
		if (document.getElementById(CC.DOM.BRIDGE_SCRIPT_ID)) {
			return (bridgeReadyPromise = Promise.resolve(true));
		}

		// FIX: Wait for a valid injection target — document_idle guarantees
		// document.head exists, but guard anyway.
		bridgeReadyPromise = new Promise((resolve) => {
			const inject = () => {
				const target = document.head || document.documentElement;
				if (!target) {
					// Should not happen at document_idle, but be safe
				...
				script.onerror = () => {
					console.warn('[ClaudeCounter] bridge.js failed to load');
					resolve(false);
				};
